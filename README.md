...
- Mesh의 해상도와 크기에 따라서 정점 간의 거리가 1보다 작을 수 있기 때문에 정점이 받는 힘을 $\1/((1+a〖x)〗^2)$ (a=감쇠(attenuation) 계수, x=거리) 형태로 설정한다. 탄성력은 F = kx(k= 탄성계수, x=변한 길이)이므로 정점의 복원력은 정점의 현재 위치가 정점의 초기 위치와 떨어진 만큼 증가시킨다. 정점의 탄성력은 시간이 흐를수록 감쇠(damping)한다.
